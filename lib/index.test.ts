...
test('generates 35-day calendar for September 2025', () => {
  const result = month(9, 2025);

  // Should return exactly 35 days
  ...
  // Last day should be October 4th, 2025
  assert.deepStrictEqual(result[34], { day: 4, month: 10, year: 2025 });
});
...
test('handles month boundaries correctly', () => {
  ...
  assert.strictEqual(jan2024.length, 35);
...
test('handles leap year February correctly', () => {
  ...
  assert.strictEqual(feb2024.length, 35);
...
test('handles non-leap year February correctly', () => {
  ...
  assert.strictEqual(feb2023.length, 35);
